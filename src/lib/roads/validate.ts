...
export function validateRoad(input: {
  ...
  if (base !== undefined) {
    if (!Number.isFinite(base))
      errors.push("Base slope must be a number.");
    else if (base < ROAD_LIMITS.base_slope_deg.min || base > ROAD_LIMITS.base_slope_deg.max)
      errors.push(`Base slope must be between ${ROAD_LIMITS.base_slope_deg.min}° and ${ROAD_LIMITS.base_slope_deg.max}° (got ${base}°).`);
  }
  ...
  for (const [i, s] of input.slopes.entries()) {
    ...
    if (s.length_m <= 0)
      errors.push(`Slope #${i + 1}: length must be positive.`);
    if (s.transition_m < 0)
      errors.push(`Slope #${i + 1}: transition length cannot be negative.`);
    if (s.bank_deg < ROAD_LIMITS.bank_deg.min || s.bank_deg > ROAD_LIMITS.bank_deg.max)
      errors.push(`Slope #${i + 1}: bank must be between ${ROAD_LIMITS.bank_deg.min}° and ${ROAD_LIMITS.bank_deg.max}° (got ${s.bank_deg}°).`);
    if (s.transition_m < 0.25 * s.length_m && s.angle_deg > 8)
      warnings.push(`Slope #${i + 1}: transition length is short for a ${s.angle_deg}° grade — vehicle may lose traction.`);
  }
  const slopeTotal = input.slopes.reduce((acc, s) => acc + s.length_m + s.transition_m, 0);
  if (slopeTotal > input.length_m)
    errors.push(`Sum of slope segments (${Math.round(slopeTotal)} m) exceeds road length (${input.length_m} m).`);
  ...
  for (let i = 0; i < sorted.length; i++) {
    const c = sorted[i];
    const minR =
      c.type === "hairpin_left" || c.type === "hairpin_right"
        ? 10
        : c.type === "banked"
          ? 30
          : 20;
    ...
    if (c.length_m <= 0)
      errors.push(`Curve #${i + 1}: length must be positive.`);
    if (!Number.isFinite(c.bank_deg) || c.bank_deg < ROAD_LIMITS.bank_deg.min || c.bank_deg > ROAD_LIMITS.bank_deg.max)
      errors.push(`Curve #${i + 1}: bank must be between ${ROAD_LIMITS.bank_deg.min}° and ${ROAD_LIMITS.bank_deg.max}° (got ${c.bank_deg}°).`);
    ...
    if (i > 0) {
      ...
      if (c.station < prevEnd)
        errors.push(`Curve #${i + 1} overlaps previous curve (starts at ${c.station} m, previous ends at ${Math.round(prevEnd)} m).`);
    }
    ...
    if (c.angle_deg > maxDeltaDeg + 1)
      warnings.push(`Curve #${i + 1}: angle (${c.angle_deg}°) exceeds arc capacity (${maxDeltaDeg.toFixed(1)}°) — spline will be clipped.`);
  }
