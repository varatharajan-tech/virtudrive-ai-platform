  ...
  return (
    <div className="p-8">
      ...
      <div className="grid md:grid-cols-3 gap-4 mb-8">
        {kpis.map((k) => (
          <Link key={k.label} to={k.to} className="panel p-5 hover:border-primary/60 transition-colors">
            <div className="flex items-center justify-between">
              <span className="text-xs uppercase tracking-widest text-muted-foreground">{k.label}</span>
              <k.icon className="w-4 h-4 text-muted-foreground" />
            </div>
            <div className="mt-2 text-3xl font-semibold num">{k.value}</div>
          </Link>
        ))}
      </div>
            ...
            {sims.map((s) => {
              ...
              return (
                <Link
                  ...
                  to="/simulations/$id"
                  params={{ id: s.id }}
                  className="grid grid-cols-12 gap-3 py-3 items-center hover:bg-accent/5 rounded-md px-2"
                >
                  ...
                  <div className="col-span-2 text-right text-xs num">
                    {summary?.top_speed_kmh?.toFixed(0) ?? "—"} km/h
                  </div>
                  <div className="col-span-1 text-right text-xs num">
