            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  ...
                  <div className="font-semibold mt-1 group-hover:text-primary transition-colors">{v.manufacturer ? `${v.manufacturer} ${v.name}` : v.name}</div>
                </div>
                {v.is_public ? (
                  <span className="inline-flex items-center gap-1 text-[10px] uppercase tracking-widest text-primary"><Sparkles className="w-3 h-3" /> seeded</span>
                ) : (
