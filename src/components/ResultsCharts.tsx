...
export interface SampleRow {
  s_m: number; speed_kmh: number; lat_g: number; long_g: number; safety_score: number; fuel_lps: number;
}
...
export function ResultsCharts({ samples }: { samples: SampleRow[] }) {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <Chart title="Longitudinal acceleration (g)" data={data} y="long_g" color="oklch(0.62 0.18 260)" />
...
function Chart({ title, data, y, color }: { title: string; data: (SampleRow & { s_km: number })[]; y: keyof SampleRow; color: string }) {
  return (
    <div>
      ...
      <div className="h-56">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 10, bottom: 0, left: -20 }}>
            ...
            <XAxis dataKey="s_km" stroke="oklch(0.7 0.02 240)" fontSize={11} tickFormatter={(v) => `${v} km`} />
            <YAxis stroke="oklch(0.7 0.02 240)" fontSize={11} width={50} />
            <Tooltip contentStyle={{ background: "oklch(0.22 0.02 240)", border: "1px solid oklch(0.32 0.02 240)", fontSize: 12 }} labelFormatter={(v) => `${v} km`} />
