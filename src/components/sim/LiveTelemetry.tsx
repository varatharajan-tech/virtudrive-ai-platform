...
const CHANNELS: Channel[] = [
  ...
  { key: "safe", label: "Adaptive safe speed", color: "#22c55e", unit: "km/h",
    transform: (s) => (s.safe_speed_mps ?? s.speed_mps) * 3.6 },
  { key: "target_delta", label: "Safe vs target (%)", color: "#a78bfa", unit: "%",
    transform: (s, ctx) => ctx.targetMps > 0 ? ((s.safe_speed_mps ?? s.speed_mps) / ctx.targetMps) * 100 : 0 },
  { key: "lat_g", label: "Lateral acceleration", color: "#f5b042", unit: "g",
    transform: (s) => s.lat_accel / G },
  { key: "long_g", label: "Longitudinal acceleration", color: "#8b5cf6", unit: "g",
    transform: (s) => s.long_accel / G },
  { key: "friction_usage", label: "Road friction usage", color: "#ef4444", unit: "%",
    transform: (s, ctx) => {
      const mu = Math.min(ctx.vehicleMu, ctx.roadMu);
      return Math.min(200, ((s.lat_accel / G) / Math.max(0.05, mu)) * 100);
    } },
  { key: "roll_margin", label: "Rollover margin", color: "#0ea5e9", unit: "%",
    transform: (s, ctx) => Math.max(0, Math.min(100, (1 - (s.lat_accel / G) / Math.max(0.5, ctx.ssf)) * 100)) },
  { key: "grip_remaining", label: "Tire grip remaining", color: "#14b8a6", unit: "%",
    transform: (s, ctx) => {
      const mu = Math.min(ctx.vehicleMu, ctx.roadMu);
      return Math.max(0, Math.min(100, (1 - (s.lat_accel / G) / Math.max(0.05, mu)) * 100));
    } },
  { key: "fuel", label: "Fuel rate", color: "#f97316", unit: "L/s",
    transform: (s) => s.fuel_rate_lps },
  { key: "safety", label: "Safety score", color: "#22c55e", unit: "/100",
...
export function LiveTelemetry({
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      {CHANNELS.map((c) => (
        <ChannelChart key={c.key} samples={samples} channel={c} totalT={totalT} ctx={ctx} targetKmh={targetKmh ?? null} />
      ))}
    </div>
  );
}
...
function ChannelChart({
  samples, channel, totalT, ctx, targetKmh,
}: {
  samples: PathSample[]; channel: Channel; totalT: number; ctx: ChannelCtx; targetKmh: number | null;
}) {
    ...
    const draw = (progress: number) => {
      ...
      if (safeOverlay) {
        ...
        for (let i = 0; i < iMax; i++) {
          const x = xAt(i), y = yAt(safeOverlay[i]);
      ...
      c.lineWidth = 1;
      c.beginPath(); c.moveTo(px, 0); c.lineTo(px, h - 12); c.stroke();
